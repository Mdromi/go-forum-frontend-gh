...
export const fetchPosts = () => {
    return (dispatch) => {
        try {
            const res = axios.get(`${API_ROUTE}/posts`)
            dispatch({type: FETCH_POSTS, payload: res.data.response})
        } catch(err) {
            dispatch({type: FETCH_POSTS_ERROR, payload: err.respone ? err.response.data.error : ""})
        }
    }
}
...
export const createPost = (createPost) => {
    return async (dispatch) => {
        ...
        try {
            ...
            dispatch({type: CREATE_POST_SUCCESS, payload: res.data.response})
            history.push('/')
        }catch(err) {
...
export const updatePost = (updateDetails, updateSuccess) => {
    return async (dispatch) => {
        ...
        try {
            ...
            dispatch({type: UPDATE_POST_SUCCESS, payload: res.data.response})
            updateSuccess()
        } catch(err) {
